/**
 * The audit page's read, kept out of the page module so it can be exercised on its own.
 *
 * What the loader answers is the whole of this screen's honesty: the range, the action filter, the
 * search and now the saved view are all applied in the query, and every count on the page is a
 * count over that query rather than over the fifty rows it happened to return.
 */
...
export type AuditQuery = {
  ...
  ascending: boolean;
  /**
   * Which saved view the reader is in, applied here rather than in the browser.
   *
   * The four views used to filter and count the loaded page: a reader in Pauses saw the pauses
   * that happened to be among the last fifty events, under a number that changed as they paged.
   * A view is the same kind of thing as the action filter and the range, so it is answered the
   * same way -- one clause on the row query, one on each view's count.
   */
  view: AuditViewKey;
};
...
export function requestedQuery(params: Record<string, string | string[] | undefined>): AuditQuery {
  ...
  return {
    range,
    view: isAuditViewKey(firstParam(params.view)) ? firstParam(params.view) as AuditViewKey : "all",
...
export async function loadAuditRows(input: AuditQuery): Promise<{
  rows: AdminAuditRow[];
  ...
  /**
   * The saved view as one more clause, kept separate from the shared filters because each view's
   * own count needs the other three clauses and a different one of these.
   */
  /*
   * One count per view, over the whole window rather than over the loaded page, so the number on a
   * segment answers "how much of this happened" instead of "how much of this landed on page one".
   * Four head reads with no rows in them, run together, and the active view's count is also the
   * total the pager works from.
   */
  const counted = await Promise.all(AUDIT_VIEWS.map(async (view) => {
    let countQuery = client.from("audit_log").select("id", { count: "exact", head: true });
    countQuery = withVisibleAuditFilters(countQuery);
    const viewFilter = auditViewFilter(view.key);
    if (viewFilter) countQuery = countQuery.or(viewFilter);
    return [view.key, await countQuery] as const;
  }));
  const countError = counted.find(([, result]) => result.error)?.[1].error ?? null;
  if (countError) {
    return {
      rows: [],
      pagination: emptyPagination,
  ...
  const viewCounts = Object.fromEntries(
    counted.map(([key, result]) => [key, result.count ?? 0]),
  ) as Record<AuditViewKey, number>;
  const count = viewCounts[input.view];
  ...
  async function executeRowsQuery(includeOrigin: boolean) {
    ...
    const viewFilter = auditViewFilter(input.view);
    if (viewFilter) query = query.or(viewFilter);
  ...
  if (result.error) {
    return {
      rows: [],
      pagination: { ...emptyPagination, totalRows: count },
      unavailableReason: "Audit events are temporarily unavailable.",
      liveWorkspaceCount,
      viewCounts,
    };

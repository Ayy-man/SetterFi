...
/**
 * The marketplace install surface, folded out of `/admin/provisioning` and into the Clients
 * screen's Setup tab.
 *
 * Everything below is the arm that page used to run, moved rather than rewritten: the same guard
 * order, the same honest-absence rules, the same three components. It lives in its own module
 * because the Clients page is a client-book read first, and a reader who opens Setup should not
 * pay for these four service-role reads on every other tab.
 */
...
/**
 * A failed custody read is never presented as an empty custody row -- and a failed receipt read is
 * not presented as a grant with nothing to say about itself either.
 *
 ...
 * screen lets through. The guard is therefore the whole gate, and the reads live *inside* its
 * branch rather than after it: an edit that moves a line cannot quietly restore the
 * read-then-authorize order that made sixty audit rows from every tenant readable by anyone the
 * proxy admitted.
 */
async function installSection(input: {
  ...
  // anyone this screen admits, so the guard above is the whole gate and nothing may run before it.
...
/**
 * The Setup tab's body. `chrome="embedded"` because the Clients screen has already drawn the rail,
 * the crumbs and the one `<h1>` this route is allowed.
 */
export async function ClientsSetupSection({ searchParams }: { searchParams: PageSearchParams }) {

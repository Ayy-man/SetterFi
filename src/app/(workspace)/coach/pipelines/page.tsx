 ...
 * on the board, the switch writes `?view=` from there, and `workspace-navigation.ts` already
...
async function liveCoachContext() {
  ...
  if (error || !data?.claims) redirect("/login?next=%2Fcoach%2Fpipelines");
...
export default async function CoachPipelinesPage() {
  if (!phase1Live()) {
    return (
      <LeadsShell>
        <DataState

...
export default async function TenantTermsPage({ params }: { params: Promise<{ tenantSlug: string }> }) {
  ...
  if (enabled) {
    const response = await loadHostedArtifact(
      new Request(`https://setterfi.local/api/opt-in/${encodeURIComponent(tenantSlug)}/artifact?page=terms`),
  ...
  return <OptinArtifact artifact={artifact} enabled={enabled} page="terms" tenantSlug={tenantSlug} />;
}

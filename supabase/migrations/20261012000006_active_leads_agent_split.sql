-- The coach home page shows one "Active leads" bubble. The Home artboard round-2 pass asks for
-- two: how many active leads the agent is currently handling on its own, and how many need the
-- coach. `conversations.status` (agent/needs_human/human/nurture/closed/opted_out,
-- 20260813000001_init.sql:54) already carries exactly that signal per conversation; this
-- migration reads each active-cohort contact's most recent conversation and buckets it.
--
-- Bucketing rule: `needs_human` and `human` both mean a person is the one who has to act (one
-- needs a human to step in, the other already has a human running the thread), so both count as
-- "needs you." Every other observed status (`agent`, `nurture`, `closed`, `opted_out`) and a
-- contact with no conversation row at all count as "agent handling" -- there is nothing pending
-- on the coach in any of those states. The two counts are constructed to partition the exact same
-- active-cohort population `coach.active_leads` already counts, so they always sum to it; the
-- read path (`src/lib/repositories/analytics.ts`) enforces that as a conservation check the same
-- way it already does for the keyword table.
--
-- `read_coach_measurement_pre_phase13` is redefined again (never editing an applied migration) to
-- add the split without touching any existing key. `read_coach_measurement`
-- (20261007000001_keyword_goals_capi.sql) passes every non-keyword metric row through unchanged,
-- so the two new keys reach the wrapper and `read_coach_measurement_for_actor` untouched.
...
declare
  ...
  active_agent_handling_count bigint := 0;
  active_needs_you_count bigint := 0;
...
begin
  ...
  -- Same active-cohort population as active_count above (nonterminal pipeline_stage), bucketed by
  -- each contact's most recent conversation status. A contact with no conversation row counts as
  -- agent-handling, same as `agent`/`nurture`/`closed`/`opted_out`; only `needs_human`/`human`
  -- count as needing the coach. The two filters partition the same rows, so they always sum to
  -- active_count.
  with active_cohort as (
    ...
    where contact.tenant_id = p_expected_tenant
      and contact.created_at >= window_start and contact.created_at < window_end
      and contact.merged_into_contact_id is null
      and contact.pipeline_stage not in ('booked', 'qualified_no_buy', 'disqualified')
  )
  select
    count(*) filter (where coalesce(latest.status, 'agent') not in ('needs_human', 'human'))::bigint,
    count(*) filter (where coalesce(latest.status, 'agent') in ('needs_human', 'human'))::bigint
  into active_agent_handling_count, active_needs_you_count
  from active_cohort
  left join lateral (
    select conversation.status::text as status
    from public.analytics_conversations conversation
    where conversation.contact_id = active_cohort.contact_id
  ...
  metrics := jsonb_build_array(
    ...
    jsonb_build_object('metricKey','coach.active_leads_agent_handling','numerator',active_agent_handling_count,'denominator',active_count,'value',active_agent_handling_count,'state',cohort_state,'windowStart',window_start,'windowEnd',window_end),
    jsonb_build_object('metricKey','coach.active_leads_needs_you','numerator',active_needs_you_count,'denominator',active_count,'value',active_needs_you_count,'state',cohort_state,'windowStart',window_start,'windowEnd',window_end),
